...
class ReplayTask extends Task{
	...
	public function __construct(private Player $player, Player $target, Main $main){
		...
		foreach($main->saved[$target->getName()]["preBlocks"] as $hash => $block){
            foreach ($block as $b) {
                ...
                $player->getNetworkSession()->sendDataPacket(UpdateBlockPacket::create(
                    ...
                    $typeConverter->getBlockTranslator()->internalIdToNetworkId($b->getStateId()),
		...
		$player->getNetworkSession()->sendDataPacket(AddPlayerPacket::create(
			...
			UpdateAbilitiesPacket::create(new AbilitiesData(CommandPermissions::NORMAL, PlayerPermissions::VISITOR, $this->eid, [])),
            [],
			"",
			DeviceOS::UNKNOWN
		));
        $skinConvertor = TypeConverter::getInstance()->getSkinAdapter()->toSkinData($target->getSkin());
		$player->getNetworkSession()->sendDataPacket(PlayerSkinPacket::create($uuid, "", "", $skinConvertor));
	}
		...
		if(count($this->list) <= 0){
			if($this->started){
				...
				foreach($this->setBlocks as $block){
                    ...
                    foreach ($block as $b) {
                        $typeConverter = $this->player->getNetworkSession()->getTypeConverter();
